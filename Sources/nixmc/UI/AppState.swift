...
final class AppState: ObservableObject {
    ...
    func installNix() {
        run {
            self.out("Installing Nix via Determinate…")
            let ok = await NixInstaller.install { line in Task { @MainActor in self.out(line) } }
            self.out(ok ? "Nix installed." : "Nix install failed.")
            self.refresh()
        }
    }
