...
struct ContentView: View {
    ...
    var body: some View {
        NavigationSplitView {
            ...
            Group {
                ...
                case .needsNix: bootstrapCard(
                    title: "Install Nix",
                    message: "NixMC needs Nix (Determinate). This requires administrator approval.",
                    icon: "shippingbox.fill",
                    button: "Install Nix", action: app.installNix)
    ...
    private var inputBar: ComposerBar {
        ComposerBar(draft: $draft, focused: $inputFocused, busy: app.busy, canStop: app.canStop,
                    onTemplates: { app.showTemplates = true }, onSubmit: submit, onStop: { app.stop() })
    }
    ...
    private func bootstrapCard(title: String, message: String, icon: String, button: String, action: @escaping () -> Void) -> some View {
        VStack(spacing: 18) {
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(Theme.brand)
                .frame(width: 72, height: 72)
                .overlay(Image(systemName: icon).font(.system(size: 32, weight: .semibold)).foregroundStyle(.white))
                .shadow(color: Theme.accent.opacity(0.24), radius: 8, y: 3)
            Text(title).font(.system(.title, design: .rounded).weight(.bold))
            Text(message).foregroundStyle(.secondary).multilineTextAlignment(.center).frame(maxWidth: 440)
            Button(button, action: action).buttonStyle(.brand).disabled(app.busy)
