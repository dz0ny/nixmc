...
nixmc installs Determinate Nix during first-time setup using Determinate's
signed macOS package. It uses the agent CLI already installed on the Mac, so no
AI API key is entered into nixmc.
...
1. Open nixmc and install Nix when prompted, if required.
...
|---|---|
| Administrator approval | Install Nix, create `/etc/nix-darwin`, and activate system changes |
